...
public class CameraAdapter extends BaseAdapter{
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        for (int j=0; j<item.resolutionList.size(); j++) {
            ...
            desc = String.format("%s分辨率%d为: 宽%d x 高%d\n", desc, j+1, size.width, size.height);
        }
        holder.tv_resolution_list.setText(desc);
        listenerClick(holder);
        return convertView;
    }

    private void listenerClick(final ViewHolder holder) {
        holder.tv_resolution.setOnClickListener(new View.OnClickListener() {
            @Override
